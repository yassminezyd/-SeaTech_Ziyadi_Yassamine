...
static uint8_t Accelerometre_AccelerometreMesuresProps = GATT_PROP_READ | GATT_PROP_WRITE | GATT_PROP_WRITE_NO_RSP | GATT_PROP_NOTIFY;
...
bStatus_t Accelerometre_AddService( void )
{
  ...
  GATTServApp_InitCharCfg( INVALID_CONNHANDLE, Accelerometre_AccelerometreMesuresConfig );
...
void SendAccelerometreMesure(void){
	...
	Accelerometre_AccelerometreMesuresVal[indexArr++] = 0x00;
	Accelerometre_AccelerometreMesuresVal[indexArr++] = 0x00;
	Accelerometre_SetParameter(ACCELEROMETRE_ACCELEROMETREMESURES,ACCELEROMETRE_ACCELEROMETREMESURES_LEN,
							   Accelerometre_AccelerometreMesuresVa1);
}
