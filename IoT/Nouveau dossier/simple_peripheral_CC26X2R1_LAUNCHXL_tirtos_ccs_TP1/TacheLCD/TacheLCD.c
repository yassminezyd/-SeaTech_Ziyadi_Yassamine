...
void afficherDonnees(float Voltaccx, float Voltaccy, float Voltaccz, float Joystick_V, float Joystick_H){
    Ax = 9.81*((Voltaccx - 1.65)/0.66)*1000;
    Ay = 9.81*((Voltaccy - 1.65)/0.66)*1000;
    Az = 9.81*((Voltaccz - 1.65)/0.66)*1000;
   // Y = ((Joystick_V - 1.65)/0.66);
   // X = ((Joystick_H - 1.65)/0.66);
    Y = Joystick_V-(3.3/2);
    X = Joystick_H-(3.3/2);
...
static void TacheLCD_taskFxn(UArg a0, UArg a1){
    ...
    for(;;){
        ...
        OLEDText22(58, 8, axDataLCD, SIZE_TWO, 0xFF, 0xFF, 0x00);

        floatToString1d(ayDataLCD, Ay);
        OLEDText22(58, 33, ayDataLCD, SIZE_TWO, 0xFF, 0xFF, 0x00);

        floatToString1d(azDataLCD, Az);
        OLEDText22(58, 58, azDataLCD, SIZE_TWO, 0xFF, 0xFF, 0x00);

        floatToString1d(jvDataLCD, Y);
        OLEDText22(58, 83, jvDataLCD, SIZE_TWO, 0xFF, 0xFF, 0x00);

        floatToString1d(jhDataLCD, X);
        OLEDText22(58, 108, jhDataLCD, SIZE_TWO, 0xFF, 0xFF, 0x00);
    }
}
